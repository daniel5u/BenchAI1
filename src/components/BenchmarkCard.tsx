    ...
    >
      <div className="flex justify-between items-start mb-1	">
        ...
        {totalHeat > 0 && (
          <div className="flex items-center gap-1 text-[11px] font-black text-orange-500 bg-orange-50 px-2 py-1 rounded-md">
            ...
            <span className="Leading-none">{totalHeat.toLocaleString()}</span>
          </div>
        )}
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        {tags.map((tag) => (
          <span
            key={tag}
            className="px-2 py-0.5 bg-slate-100 text-slate-500 text-[10px] font-bold uppercase rounded border border-slate-200"
          >
            {tag}
          </span>
        ))}
